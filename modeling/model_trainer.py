...
class ModelTrainer:
    """
    Class that aggregates a set of functions to perform feature selection, train and
    evaluate the models and store the results. Notice that all classifiers used must
    be compatible with scikit-learn API. This class also requires the passing of three
    ...
    def __init__(self, X_train, y_train, X_val, y_val, X_test, y_test, seed):
    ...
    def train_classifier(
        ...
        if perform_feature_selection:
            self.perform_feature_selection(
                algorithm=feature_selection_algorithm, num_features=num_features
            )
    ...
    def evaluate_classifier(
        ...
        def calculate_metrics(y_true, y_preds, prefix):
            return {
                ...
                f"{prefix}_balanced_accuracy_score": round(
                    balanced_accuracy_score(y_true, y_preds), 2
                ),
            }

        assert classifier_name in self.models, (
            f"There is no model registered under " f"the name {classifier_name}."
        )
    ...
    def perform_feature_selection(
        self, algorithm: str = "mutual_info_classif", num_features: int = 20
    ):
        """Performs feature selection by applying 'SelectKBest' from scikit-learn
        library. It's prepared to use 1 of 2 score functions: mutual_info_classif
        and f_classif."""
        assert algorithm in ["f_classif", "mutual_info_classif"], (
            f"Expected 'mutual_info_classif' or 'f_classif', "
            f"got {algorithm} instead."
        )
    ...
    def add_classifier_metadata(self, classifier_name: str, metadata: Union[Dict, str]):
        """Allows to store some additional information about the model, namely used params,
        structure of the pipeline used to treat and prepare the data, among others."""
