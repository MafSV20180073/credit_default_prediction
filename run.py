...
if __name__ == "__main__":
    ...
    if download_from_kaggle:
        ...
        splits = {}
        train_data, val_data, test_data = pd.read_csv(f"{path}train_set_{suffix}.csv"), pd.read_csv(f"{path}val_set_{suffix}.csv"), pd.read_csv(f"{path}test_set_{suffix}.csv")
        splits["X_train"], splits["y_train"] = train_data.iloc[:, :-1], train_data.iloc[:, -1]
        splits["X_val"], splits["y_val"] = val_data.iloc[:, :-1], val_data.iloc[:, -1]
        splits["X_test"], splits["y_test"] = test_data.iloc[:, :-1], test_data.iloc[:, -1]
    ...
    data_processor.export_datasets(suffix="treated_v2", path="data/")
