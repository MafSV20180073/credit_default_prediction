...
class DataProcessor:
    ...
    val_size : float, optional
        Represents the proportion of the train dataset to include in the validation split, by default
        is 0.2.
    """
    def __init__(
            self,
        ...
        assert all(key in splits for key in ["X_train", "y_train", "X_test", "y_test", "X_val", "y_val"]), f"Splits argument must contain X_train, y_train, X_test, y_test, " \
                                                                                                           f"X_val, y_val keys."
    ...
    def treat_categorical_variables(self, drop_original_vars: bool = True):
        """Encoding of 'education' variable and creation of flags for clients who are male, married, or single."""
    ...
    def standardize_data(self):
        ...
        self.transformer = ColumnTransformer(transformers=[
            ("", MinMaxScaler(clip=True), col_names)
    ...
    def treat_outliers(self, verbose: bool = False):
        """Identifies and treats outliers using the interquartilic range criteria."""
        def detect_outliers(data: pd.DataFrame, col: str, iqr_cut_point: float, dataset_type: str):
            """Detects outliers in a column given the IQR cut point."""
            out = data[(data[col] > iqr_cut_point) | (data[col] < -iqr_cut_point)]
            if len(out):
                if verbose:
                    print(f"Number of outliers detected in '{col}' column of {dataset_type} dataset: {len(out)} ({round(len(out)/len(data) * 100,3)}% of dataset rows with values "
                          f"below or above 1.5 IQR from 1Q/3Q).")
                return True
            else:
                return False
        ...
        def remove_outliers(data: pd.DataFrame, col: str, iqr_cut_point: float):
            ...
            data[col] = data[col].apply(lambda x: iqr_cut_point if x > iqr_cut_point else (-iqr_cut_point if x < -iqr_cut_point else x))
            return data

        numeric_columns = self.X_train.select_dtypes(include=np.number).columns.tolist()

        for column in numeric_columns:
            out_cut = np.nanpercentile(self.X_train[column], 75) + (sp.stats.iqr(self.X_train[column], nan_policy='omit') * 1.5)
    ...
    def export_datasets(self, suffix: str, path: str = "../data"):
        ...
        train_data = pd.concat([self.X_train, pd.DataFrame({"target": self.y_train})], axis=1)
        val_data = pd.concat([self.X_val, pd.DataFrame({"target": self.y_val})], axis=1)
        test_data = pd.concat([self.X_test, pd.DataFrame({"target": self.y_test})], axis=1)
    ...
    def _initial_dataset_uniformization(self, dataset_type: str):
        ...
        if dataset_type == "train":
            ...
            raise ValueError(f"dataset_type arg must be 'train', 'test' or 'val', got {dataset_type} instead.")
    ...
    def _treat_categorical_variables(self, data: pd.DataFrame, drop_original_vars: bool = True):
        ...
        married, or single."""
        assert "education" in self._mappings, "There is no mapping to encode 'education' variable."
